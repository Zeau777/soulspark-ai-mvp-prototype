...
export class RealtimeChat {
  ...
  async init() {
    try {
      ...
      // Start recording to feed audio if you want manual streaming
      this.recorder = new AudioRecorder((audioData) => {
        // Example: you could use a data channel to send extra events if needed
        if (this.dc?.readyState === "open") {
          this.dc.send(
            JSON.stringify({ type: "client.debug", samples: audioData.length })
          );
        }
      });
